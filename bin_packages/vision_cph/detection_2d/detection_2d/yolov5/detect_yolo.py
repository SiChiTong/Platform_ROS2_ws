...
class detection_node(Node):
    def __init__(self, model, device):
        ...
        self.declare_parameter("downsample_rate", value=0.03)
    ...
    def callback_rgbd(self, msg: RGBDImage):
        ...
        with torch.no_grad():
            ...
            result = detect(self.device, self.model, img, self.names, self.colors,
                            imgsz=list(img.shape[0:2]), conf_thres=0.2, iou_thres=0.3, isVisualized=True)
            ...
            for a in result:
                ...
                try:
                    ...
                    if(bbox_ratio > 0.3):
                        pcd_down = aab[np.random.choice(pcd_size, round(pcd_size * self.downsample_rate*0.25), replace=False)]
                    elif(bbox_ratio > 0.2):
                        pcd_down = aab[np.random.choice(pcd_size, round(pcd_size * self.downsample_rate*0.4), replace=False)]
                    elif(bbox_ratio>0.1):
                        pcd_down = aab[np.random.choice(pcd_size, round(pcd_size * self.downsample_rate*0.7), replace=False)]
                    else:
                        pcd_down = aab[np.random.choice(pcd_size, round(pcd_size * self.downsample_rate), replace=False)]
                    ...
                    labels = np.asarray(pcd_bbox.cluster_dbscan(eps=0.3, min_points=20, print_progress=False).cpu())
        ...
        self.vis.pcd_show(pcd_total, bbox=bbox_total)
        # view(img)
