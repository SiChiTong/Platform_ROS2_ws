...
parser.add_argument('--nms_iou', type=float, default=0.25, help='NMS iou [default: 0.25]')
parser.add_argument('--conf_thresh', type=float, default=0.5, help='Confidence Threshold [default: 0.5]')
...
def load_config(FLAGS_TARGET):
    if FLAGS_TARGET.dataset == 'sunrgbd':
    ...
    elif FLAGS_TARGET.dataset == 'scannet':
        ...
        print(f'Unknown dataset {(FLAGS_TARGET.dataset)}. Exiting.')
        exit(-1)

    return DC, pc_path, checkpoint_path
...
if __name__=='__main__':
    ...
    DC, pc_path, checkpoint_path = load_config(FLAGS)

    eval_config_dict = {'remove_empty_box': True, 'use_3d_nms': True, 'nms_iou': nms_iou,
